...
export class DatabaseProvider
{
  ...
  initialize()
  {
    this.sqlite.create({
      ...
      .then((database: SQLiteObject) => {
        ...
        this.database.executeSql('CREATE TABLE IF NOT EXISTS Events (event_id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, event_name TEXT, event_date TEXT, event_date_time NUMBER, event_type TEXT, event_description TEXT, event_latitude REAL, event_longitude REAL, event_address TEXT, event_file_url TEXT);', [])
  ...
  insertEvent(event : Event)
  {
    ...
    let sqlRequest = 'INSERT INTO Events (event_name, event_date, event_date_time, event_type, event_description, event_latitude, event_longitude, event_address, event_file_url) VALUES (\'' + event.name + '\', \'' + event.date + '\', \'' + event.dateTime  + '\', \'' + event.type + '\', \'' + event.description + '\', ' + event.latitude + ', ' + event.longitude + ', \'' + event.address+ '\', \'' + event.fileURL +  '\');';
    ...
    this.database.executeSql('SELECT * FROM Events WHERE event_type = \''+ type +'\' ;', []).then((data) => {
      for (var i = 0; i < data.rows.length; i++)
      {
        ...
        events.push(
        {
          ...
          dateSecond : data.rows.item(i).event_date_time,
  ...
  removeOldEvents()
  {
    let sqlRequest = 'DELETE FROM Events WHERE event_date_time < ' + new Date().getTime() + ';';

    console.log(sqlRequest);

    this.database.executeSql(sqlRequest, [])
          .then(() => console.log("removeOldEvents"))
          .catch(error => this.showToast('Error events removing'));
  }
